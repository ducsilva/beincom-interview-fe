import { IComment, TPost } from "@/types";
...
const SinglePost = ({ post }: { post: TPost }) => {
  const { title, category, banner, content, createdAt, user, id, comments, _id } = post;
  return (
    <>
      <div className="group relative overflow-hidden rounded-xl bg-white shadow-one duration-300 hover:shadow-two dark:bg-dark dark:hover:shadow-gray-dark">
        ...
        <section className="px-4 pb-4 pt-3">
          ...
          {comments
            ...
            ?.map((comment) => {
              return (
                <CommentDetail key={comment.id} comment={comment} postId={id} />
              );
            })}
        </section>
      </div>
